...
class HierarchicalProcess(BaseProcess):
    ...
    async def execute(
        ...
        try:
            ...
            turns.append(
                self._create_turn(
                    self.manager_agent, manager_analysis, phase="analysis"
                )
            )
            ...
            for i, task in enumerate(tasks):
                ...
                delegated_agent = await self._manager_delegate_task(
                    task, worker_agents, agent_lookup
                )
                ...
                # Manager review
                review = await self._manager_review_output(
                    task, output, delegated_agent
                )
                turns.append(
                    self._create_turn(
                        self.manager_agent, review, phase="review", task_index=i
                    )
                )
                ...
                if isinstance(output, TaskOutput):
                    ...
                    tasks_output.append(TaskOutput(raw=str(output), task=task))
            ...
            turns.append(
                self._create_turn(self.manager_agent, final_summary, phase="summary")
            )
    ...
    async def _manager_analyze_tasks(self, tasks: List[LiteTask]) -> str:
        ...
        # TODO: Use prompt with LLM
        # analysis_prompt = (
        #     f"Analyze these tasks and plan the execution strategy:\n{task_descriptions}"
        # )
    ...
    async def _manager_review_output(
        self, task: LiteTask, output: Any, worker: LiteAgent
    ) -> str:
