...
class ConversationalProcess(BaseProcess):
    ...
    async def execute(
        ...
        try:
            ...
            turns.append(self._create_turn(agents[0], intro, phase="introduction"))
            ...
            while turn_count < self.max_turns:
                ...
                next_agent = await self._select_next_speaker(
                    agents, agent_turn_counts, turns
                )
                ...
                if task_output:
                    ...
                    self._emit_event(
                        "task_progress", {"agent": next_agent, "output": task_output}
                    )

                # Check if minimum turns reached for all agents
                min_reached = all(
                    count >= self.min_turns for count in agent_turn_counts.values()
                )
                if min_reached and turn_count >= len(agents) * self.min_turns:
                    if self._conversation_complete(
                        turns, tasks
                    ) or self._all_tasks_discussed(tasks, turns):
                        break

            # Final summary
            summary_agent = self._select_summary_agent(agents, agent_turn_counts)
            summary = await self._generate_conversation_summary(
                summary_agent, turns, tasks_output
            )

            turns.append(self._create_turn(summary_agent, summary, phase="summary"))
            ...
            for task in tasks:
                if not any(
                    ...
                    if hasattr(out, "task") and out.task == task
                ):
                    # Create output from conversation
                    task_result = self._synthesize_task_result(task, turns)
                    tasks_output.append(TaskOutput(raw=task_result, task=task))
    ...
    async def _select_next_speaker(
        self, agents: List[LiteAgent], turn_counts: Dict[str, int], turns: List
    ) -> LiteAgent:
        ...
        if self.turn_style == "round_robin":
            ...
            weights = [
                max(1, max(turn_counts.values()) - turn_counts[a.role] + 1)
                for a in agents
            ]
            return random.choices(agents, weights=weights)[0]

    async def _generate_conversational_response(
        self, agent: LiteAgent, turns: List, tasks: List[LiteTask], context: Dict
    ) -> str:
        """Generate agent's conversational response"""
        # Build conversation history
        # recent_turns = turns[-5:] if len(turns) > 5 else turns
        # TODO: Use history for better responses
        # history = "\n".join(
        #     f"{turn.agent}: {turn.content[:200]}..." for turn in recent_turns
        # )
    ...
    def _extract_task_output(
        self, response: str, agent: LiteAgent, tasks: List[LiteTask]
    ) -> Optional[TaskOutput]:
        ...
        if any(keyword in response.lower() for keyword in keywords):
            ...
            for task in tasks:
                if task.description.lower() in response.lower():
                    return TaskOutput(raw=response, agent_role=agent.role)
        return None
    ...
    def _select_summary_agent(
        self, agents: List[LiteAgent], turn_counts: Dict[str, int]
    ) -> LiteAgent:
    ...
    async def _generate_conversation_summary(
        self, agent: LiteAgent, turns: List, outputs: List[TaskOutput]
    ) -> str:
        ...
        summary += "we reached the following conclusions:\n\n"
    ...
    def _synthesize_task_result(self, task: LiteTask, turns: List) -> str:
        ...
        if relevant_content:
            return "Based on team discussion:\n" + "\n".join(relevant_content[-3:])
        else:
    ...
    def _build_conversation_context(self, turns: List, tasks: List[LiteTask]) -> str:
        ...
        if not turns:
            return (
                f"Starting discussion about: {', '.join(t.description for t in tasks)}"
            )
