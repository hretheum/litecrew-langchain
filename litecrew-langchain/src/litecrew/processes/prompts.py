...
As {agent_role}, help develop these ideas into actionable solutions."""
    ...
    @classmethod
    def get_prompt(cls, process_type: str, prompt_name: str, **kwargs) -> str:
        ...
        if prompt_name not in process_prompts:
            raise ValueError(f"Unknown prompt '{prompt_name}' for process '{process_type}'")
    ...
    def get_process_instructions(cls, process_type: str) -> Dict[str, Any]:
        ...
        instructions = {
            "conversational": {
                ...
                "goals": ["build on others' ideas", "seek understanding", "find common ground"],
                "avoid": ["dominating conversation", "dismissing others", "going off-topic"],
                "techniques": ["active listening", "yes-and thinking", "clarifying questions"]
            },
            "debate": {
                "tone": "formal and respectful", 
                "goals": ["present strong arguments", "address counterpoints", "persuade audience"],
                "avoid": ["personal attacks", "logical fallacies", "emotional manipulation"],
                "techniques": ["evidence-based reasoning", "structured arguments", "effective rebuttals"]
            },
            "panel": {
                "tone": "professional and expert",
                "goals": ["share domain expertise", "provide unique insights", "educate audience"],
                "avoid": ["oversimplification", "jargon overuse", "contradicting unnecessarily"],
                "techniques": ["drawing from experience", "citing examples", "clear explanations"]
            },
            "brainstorm": {
                "tone": "creative and energetic",
                "goals": ["generate many ideas", "think unconventionally", "inspire others"],
                "avoid": ["criticism during ideation", "self-censoring", "premature evaluation"],
                "techniques": ["lateral thinking", "building on ideas", "wild associations"]
            }
        }
...
def enhance_agent_for_process(agent: Any, process_type: str, role_in_process: str = None) -> None:
