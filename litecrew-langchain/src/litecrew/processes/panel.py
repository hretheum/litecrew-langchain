...
class PanelProcess(BaseProcess):
    ...
    async def execute(
        ...
        if not valid:
            return ProcessResult(raw="", success=False, error=error)
        ...
        try:
            ...
            # Opening
            opening = await self._generate_panel_introduction(
                moderator, panelists, tasks
            )
            turns.append(
                self._create_turn(
                    moderator, opening, phase="introduction", role="moderator"
                )
            )
            ...
            for task_idx, task in enumerate(tasks):
                ...
                if self.voting_enabled:
                    ...
                    turns.append(
                        self._create_turn(
                            moderator, consensus, phase="consensus", task_index=task_idx
                        )
                    )
                    ...
                    turns.append(
                        self._create_turn(
                            moderator, synthesis, phase="synthesis", task_index=task_idx
                        )
                    )
                ...
                # Create task output
                tasks_output.append(
                    TaskOutput(raw=task_result, agent_role=moderator.role)
                )
            ...
            turns.append(self._create_turn(moderator, closing, phase="closing"))
    ...
    async def _generate_panel_introduction(
        self, moderator: LiteAgent, panelists: List[LiteAgent], tasks: List[LiteTask]
    ) -> str:
    ...
    async def _moderator_introduce_topic(
        self, moderator: LiteAgent, task: LiteTask
    ) -> str:
    ...
    async def _generate_expert_opinion(
        self, panelist: LiteAgent, task: LiteTask, previous_opinions: List[tuple]
    ) -> str:
        """Generate expert opinion from panelist"""
        # Build on previous opinions if any
        context = ""
        if previous_opinions:
            context = "Building on previous perspectives, "
        ...
        opinion += "From my domain expertise, I recommend focusing on key factors that align with best practices in the field."
    ...
    async def _conduct_panel_vote(
        self, panelists: List[LiteAgent], task: LiteTask, expert_views: List[tuple]
    ) -> tuple[Dict[str, str], str]:
        """Conduct panel voting on best approach"""
        votes = {}
        options = [
            "Option A: Rapid implementation",
    ...
    async def _build_consensus(
        ...
        consensus += (
            "a collaborative approach that incorporates each expert's key insights."
        )

        return consensus

    async def _moderator_synthesize(
        self, moderator: LiteAgent, task: LiteTask, expert_views: List[tuple]
    ) -> str:
        ...
        synthesis += (
            f"1. Multiple valid approaches exist to achieve {task.expected_output}\n"
        )
        synthesis += "2. Each expert brought unique insights from their domain\n"
        synthesis += (
            "3. The recommended approach combines elements from all perspectives\n"
        )
        synthesis += "The panel's collective wisdom points to a balanced, multi-faceted solution."

        return synthesis

    async def _generate_panel_closing(
        self, moderator: LiteAgent, outputs: List[TaskOutput]
    ) -> str:
