...
class DebateProcess(BaseProcess):
    ...
    async def execute(
        ...
        try:
            ...
            if moderator:
                ...
                turns.append(
                    self._create_turn(
                        moderator, opening, phase="opening", role="moderator"
                    )
                )
            ...
            for task_idx, task in enumerate(tasks):
                ...
                tasks_output.append(
                    TaskOutput(
                        raw=resolution, agent_role=(moderator or debaters[0]).role
                    )
                )

                self._emit_event(
                    "debate_complete", {"task": task, "resolution": resolution}
                )
    ...
    def _assign_debate_positions(self, agents: List[LiteAgent]) -> Dict[str, Any]:
        ...
        if self.moderator_role:
            moderator_agent = next(
                (a for a in agents if a.role == self.moderator_role), None
            )
            if moderator_agent:
    ...
    async def _moderator_opening(
        self, moderator: LiteAgent, tasks: List[LiteTask]
    ) -> str:
    ...
    async def _generate_opening_statement(
        self, agent: LiteAgent, task: LiteTask, position: str
    ) -> str:
    ...
    async def _generate_rebuttal(
        self, agent: LiteAgent, position: str, recent_arguments: List
    ) -> str:
    ...
    async def _generate_closing_statement(
        self, agent: LiteAgent, task: LiteTask, position: str, all_turns: List
    ) -> str:
    ...
    async def _synthesize_debate_outcome(
        self, task: LiteTask, turns: List, synthesizer: LiteAgent
    ) -> str:
        """Synthesize debate into actionable outcome"""
        # Count arguments by position
        prop_args = len(
            [t for t in turns if t.metadata.get("position") == "proposition"]
        )
        ...
        synthesis += (
            "Both sides presented valid points, leading to a nuanced understanding. "
        )
        synthesis += (
            f"The recommended approach to achieve {task.expected_output} incorporates "
        )
        synthesis += "elements from both the proposition and opposition positions."
    ...
    def _create_debate_summary(self, turns: List, outputs: List[TaskOutput]) -> str:
        ...
        total_arguments = len(
            [t for t in turns if t.metadata.get("phase") == "argument"]
        )
        total_rebuttals = len(
            [t for t in turns if t.metadata.get("phase") == "rebuttal"]
        )
