...
from ..models import CrewResponse, ProcessSwitch, ProcessType
...
async def switch_crew_process(
    crew_id: str, process_switch: ProcessSwitch
) -> CrewResponse:
    ...
    if not crew_info:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Crew {crew_id} not found"
        )
    ...
    try:
        process_config = create_process_config_with_websocket(
            crew_id, process_switch.process_config
        )

        crew_instance.switch_process(process_switch.process_type, process_config)
