...
class TemplateAnalytics:
    ...
    def _update_aggregated_stats(self) -> None:
        ...
        for template in template_counts:
            ...
            if template_events:
                avg_time = sum(e["execution_time"] for e in template_events) / len(template_events)
    ...
    def get_popular_templates(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for template, count in total_usage.most_common(limit):
            popular.append({
                ...
                "avg_execution_time": round(avg_times.get(template, 0), 2) if avg_times.get(template) else None
            })
    ...
    def get_template_stats(self, template_name: str) -> Dict[str, Any]:
        ...
        events = [e for e in self.usage_data["usage_events"] if e["template_name"] == template_name]
    ...
    def get_usage_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_events = [
            e for e in self.usage_data["usage_events"]
        ...
        return {
            ...
            "unique_templates_used": len(set(e["template_name"] for e in recent_events))
        }
    ...
    def generate_insights(self) -> List[str]:
        ...
        insights.append(f"Most popular template: '{most_popular}' with {total_usage[most_popular]} uses")
        
        # Highest success rate
        if success_rates:
            best_success = max(success_rates, key=success_rates.get)
            rate = success_rates[best_success]
            insights.append(f"Highest success rate: '{best_success}' with {rate:.1f}% success")
        
        # Recent trend
        if recent_usage:
            trending = max(recent_usage, key=recent_usage.get)
            insights.append(f"Trending this week: '{trending}' with {recent_usage[trending]} recent uses")
        ...
        insights.append(f"Template diversity: {total_templates} different templates used, averaging {avg_uses_per_template:.1f} uses each")
        ...
        if success_rates:
            ...
            low_success_templates = [t for t, rate in success_rates.items() if rate < 80]
            if low_success_templates:
                insights.append(f"Templates with low success rates: {', '.join(low_success_templates)}")
