...
class AgentSelector:
    ...
    def select_agents(
        ...
        for agent_type in agent_types[:num_agents]:
            ...
            agent_config["goal"] = f"{agent_config['goal']} for: {task_description[:100]}"
        ...
        if num_agents >= 3 and not any(a["type"] == "moderator" for a in agents):
            ...
            agents[-1]["goal"] = f"Coordinate team efforts for: {task_description[:100]}"
    ...
    def suggest_process_type(cls, task_description: str, num_agents: int) -> str:
        ...
        if any(word in task_lower for word in ["debate", "discuss", "argue", "pros and cons"]):
            return "debate"
        elif any(word in task_lower for word in ["decide", "choose", "select", "evaluate options"]):
            return "panel"
        elif any(word in task_lower for word in ["brainstorm", "ideas", "creative", "innovate"]):
            return "conversational"
        elif any(word in task_lower for word in ["step by step", "sequential", "ordered", "phases"]):
            return "sequential"
        elif num_agents > 3 and any(word in task_lower for word in ["coordinate", "manage", "lead"]):
            return "hierarchical"
    ...
    def estimate_execution_time(cls, task_description: str, num_agents: int) -> int:
        ...
        if any(word in task_lower for word in ["research", "analyze", "investigate"]):
            complexity_time += 60
        if any(word in task_lower for word in ["comprehensive", "detailed", "thorough"]):
            complexity_time += 60
