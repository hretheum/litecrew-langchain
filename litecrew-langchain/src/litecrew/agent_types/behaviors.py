...
class CriticalThinkingBehavior(AgentBehavior):
    ...
    def modify_prompt(self, prompt: str, context: Dict[str, Any]) -> str:
        ...
        instruction = critical_instructions.get(self.level, critical_instructions["moderate"])
        return f"{prompt}\n\n{instruction}"
    
    def modify_response(self, response: str, context: Dict[str, Any]) -> str:
        """Ensure response includes critical elements."""
        critical_phrases = ["however", "but", "although", "on the other hand", "could be improved"]
...
class VerboseThinkingBehavior(AgentBehavior):
    ...
    def modify_prompt(self, prompt: str, context: Dict[str, Any]) -> str:
        ...
        closest_key = min(verbosity_instructions.keys(), 
                         key=lambda x: abs(x - self.verbosity))
...
class ModerationBehavior(AgentBehavior):
    ...
    def modify_prompt(self, prompt: str, context: Dict[str, Any]) -> str:
        ...
        instruction = moderation_instructions.get(self.style, moderation_instructions["balanced"])
...
class ConversationalBehavior(AgentBehavior):
    ...
    def modify_response(self, response: str, context: Dict[str, Any]) -> str:
        ...
        if self.style == "friendly" and not any(word in response.lower() for word in ["great", "excellent", "interesting"]):
