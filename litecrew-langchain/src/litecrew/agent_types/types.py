...
class CriticAgent(AgentType):
    ...
    def process_response(self, response: str, context: Dict[str, Any]) -> str:
        ...
        has_positive = any(word in response.lower() for word in 
                          ["good", "excellent", "strong", "effective", "well"])
        has_negative = any(word in response.lower() for word in 
                          ["however", "but", "could", "should", "improve", "issue"])
