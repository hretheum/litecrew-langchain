    ...
    def __init__(
        ...
        if self.type:
            ...
            if not self._validate_agent_type(self.type):
                raise ValueError(
                    f"Invalid agent type: {self.type}. Available types: {AgentTypeFactory.list_types()}"
                )
...
{{agent_scratchpad}}"""
    ...
    def _apply_type_to_prompt(self, prompt: str, context: Dict[str, Any]) -> str:
        ...
        if self._behavior_modifier:
            enhanced_prompt = self._behavior_modifier.apply_to_prompt(
    ...
    def _apply_type_to_response(self, response: str, context: Dict[str, Any]) -> str:
        ...
        # Apply behavior modifications
        if self._behavior_modifier:
            processed_response = self._behavior_modifier.apply_to_response(
        ...
        if not self._agent_type.validate_response(processed_response):
            ...
            if (
                self._agent_type.config.require_reasoning
                and "because" not in processed_response.lower()
            ):
                processed_response += "\n\nThis conclusion is reached because of the analysis provided above."

            if (
                self._agent_type.config.min_response_length
                and len(processed_response)
                < self._agent_type.config.min_response_length
            ):
    ...
    def get_type_info(self) -> Dict[str, Any]:
        ...
        if not self.type:
            return {
                "type": None,
                "description": "Standard agent without specialized type",
            }
        ...
        if self._agent_type:
            type_info["personality"] = {
                "traits": [
                    t.value for t in self._agent_type.personality.primary_traits
                ],
    ...
    def validate_type_config(self) -> bool:
        ...
        if self.type == "critic" and config.criticism_level not in [
            "mild",
            "moderate",
            "harsh",
            None,
        ...
        if self.type == "moderator" and config.moderation_style not in [
            "strict",
            "balanced",
            "permissive",
            None,
        ...
        if self.type == "conversational" and config.conversation_style not in [
            "formal",
            "friendly",
            "casual",
            "adaptive",
