    ...
    def _validate_value(self, value: Any, schema: Dict[str, Any], path: str) -> bool:
        ...
        if schema_type == "object":
            if not isinstance(value, dict):
                self._errors.append(f"{path}: Expected object, got {type(value).__name__}")
                return False
            ...
            if not isinstance(value, list):
                self._errors.append(f"{path}: Expected array, got {type(value).__name__}")
                return False
            ...
            if not isinstance(value, str):
                self._errors.append(f"{path}: Expected string, got {type(value).__name__}")
                return False
            ...
            if not isinstance(value, (int, float)):
                self._errors.append(f"{path}: Expected number, got {type(value).__name__}")
                return False
            
            # Check range
            if "minimum" in schema and value < schema["minimum"]:
                self._errors.append(f"{path}: Value {value} is below minimum {schema['minimum']}")
            if "maximum" in schema and value > schema["maximum"]:
                self._errors.append(f"{path}: Value {value} is above maximum {schema['maximum']}")
            
            return len(self._errors) == 0
        
        elif schema_type == "integer":
            if not isinstance(value, int) or isinstance(value, bool):
                self._errors.append(f"{path}: Expected integer, got {type(value).__name__}")
                return False
...
class OutputFormatter:
    ...
    def __init__(self, output_format: str = "json", custom_formatter: Optional[Any] = None):
