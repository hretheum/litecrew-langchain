        ...
        try:
            with self._get_connection() as conn:
                ...
                self._metrics.average_read_time_ms = (
                    self._metrics.average_read_time_ms * (self._metrics.total_reads - 1)
                    + read_time
    ...
    def write(self, key: str, data: Dict[str, Any], compress: bool = False) -> None:
        ...
        try:
            ...
            with self._get_connection() as conn:
                # Get current version
                cursor = conn.execute(
                    "SELECT latest_version FROM storage_metadata WHERE key = ?", (key,)
                )
                row = cursor.fetchone()
                ...
                if compress:
                    ...
                    self._metrics.compression_ratio = (
                        self._metrics.compression_ratio * 0.9
                        + (size_bytes / original_size) * 0.1
                    )
