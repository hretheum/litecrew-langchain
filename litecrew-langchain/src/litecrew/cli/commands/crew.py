...
def create(ctx, crew_file, name, dry_run):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            click.echo(click.style(f"✅ Crew '{crew_data['name']}' created successfully", fg='green'))
            ...
            click.echo(click.style(f"❌ Failed to create crew: {error_data.get('detail', 'Unknown error')}", fg='red'))
            sys.exit(1)
...
def list(ctx, output_format, name_filter):
    ...
    try:
        ...
        # Apply filter
        if name_filter:
            crews = [c for c in crews if name_filter.lower() in c.get('name', '').lower()]
...
@click.option('--inputs', type=click.Path(exists=True), help='JSON file with execution inputs')
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if async_exec:
                ...
                click.echo(click.style(f"✅ Execution started (ID: {execution_id})", fg='green'))
                
                if wait:
                    click.echo("⏳ Waiting for completion...")
                    # TODO: Implement polling for completion
                    click.echo("Note: Polling not yet implemented. Use 'litecrew export execution <id>' to check status.")
                else:
                    click.echo("Use 'litecrew export execution <id>' to check status and results.")
            else:
                ...
                if verbose:
                    ...
                    if isinstance(result, dict):
                        click.echo(f"Result: {result.get('result', 'No result')[:200]}...")
                    else:
                        click.echo(f"Result: {str(result)[:200]}...")
        else:
            error_data = response.json() if response.text else {}
            click.echo(click.style(f"❌ Execution failed: {error_data.get('detail', 'Unknown error')}", fg='red'))
            sys.exit(1)
...
def delete(ctx, crew_id):
    ...
    try:
        ...
        if response.status_code == 204:
            click.echo(click.style(f"✅ Crew {crew_id} deleted successfully", fg='green'))
        elif response.status_code == 404:
            click.echo(click.style(f"❌ Crew {crew_id} not found", fg='red'))
            sys.exit(1)
        else:
            click.echo(click.style(f"❌ Failed to delete crew: HTTP {response.status_code}", fg='red'))
            sys.exit(1)
...
def show(ctx, crew_id, output_format):
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            click.echo(click.style(f"❌ Failed to fetch crew: HTTP {response.status_code}", fg='red'))
            sys.exit(1)
        ...
        if output_format == 'json':
            ...
            for i, agent in enumerate(crew_data.get('agents', []), 1):
                ...
                click.echo(f"     Backstory: {agent.get('backstory', 'No backstory')[:50]}...")
            ...
            for i, task in enumerate(crew_data.get('tasks', []), 1):
                ...
                click.echo(f"     Expected Output: {task.get('expected_output', 'None')[:30]}...")
