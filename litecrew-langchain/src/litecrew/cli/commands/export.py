...
@click.option('--format', 'output_format', type=click.Choice(['json', 'yaml', 'txt', 'csv']), default='json', help='Output format')
...
def execution(ctx, execution_id, output_format, output, include_metadata):
    ...
    try:
        with httpx.Client() as client:
            response = client.get(f"{api_url}/api/v1/executions/{execution_id}", timeout=10.0)
        
        if response.status_code == 404:
            click.echo(click.style(f"❌ Execution {execution_id} not found", fg='red'))
            sys.exit(1)
        elif response.status_code != 200:
            click.echo(click.style(f"❌ Failed to fetch execution: HTTP {response.status_code}", fg='red'))
            sys.exit(1)
        ...
        if output:
            ...
            click.echo(click.style(f"✅ Execution results exported to {output}", fg='green'))
        else:
...
def executions(ctx, crew_id, output_format, output, limit):
    ...
    try:
        ...
        if output_format == 'json':
            ...
            flattened_executions = [_flatten_dict(exec_data) for exec_data in executions]
        ...
        if output:
            with open(output, 'w') as f:
                f.write(formatted_data)
            click.echo(click.style(f"✅ {len(executions)} executions exported to {output}", fg='green'))
        else:
            click.echo(formatted_data)
...
@click.option('--format', 'output_format', type=click.Choice(['json', 'yaml', 'csv']), default='json', help='Output format')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--include-config', is_flag=True, help='Include crew configuration details')
@click.pass_context
def crews(ctx, output_format, output, include_config):
    ...
    try:
        ...
        if output:
            with open(output, 'w') as f:
                f.write(formatted_data)
            click.echo(click.style(f"✅ {len(export_crews)} crews exported to {output}", fg='green'))
        else:
            click.echo(formatted_data)
...
def metrics(ctx, output):
    ...
    try:
        with httpx.Client() as client:
            ...
            executions_response = client.get(f"{api_url}/api/v1/executions", timeout=10.0)
        ...
        crews_data = crews_response.json() if crews_response.status_code == 200 else {'crews': []}
        executions_data = executions_response.json() if executions_response.status_code == 200 else {'executions': []}
        ...
        metrics_data = {
            ...
            "usage": {
                ...
                "completed_executions": len([e for e in executions_data.get('executions', []) if e.get('status') == 'completed'])
            }
        }
        ...
        if output:
            with open(output, 'w') as f:
                f.write(formatted_data)
            click.echo(click.style(f"✅ System metrics exported to {output}", fg='green'))
        else:
            click.echo(formatted_data)
        ...
        if verbose and not output:
            ...
            click.echo(f"   Total Executions: {metrics_data['usage']['total_executions']}")
