    ...
    try:
        ...
        with httpx.Client() as client:
            response = client.post(
                f"{api_url}/api/v1/crews/{crew_id}/tasks", json=task_data, timeout=10.0
            )
        ...
        if response.status_code == 202:
            ...
            click.echo(
                click.style(
                    f"✅ Task submitted successfully (ID: {task_id})", fg="green"
                )
            )
            ...
            click.echo(
                click.style(
                    f"❌ Failed to submit task: {error_data.get('detail', 'Unknown error')}",
                    fg="red",
                )
            )
            sys.exit(1)
...
def status(ctx, task_id, wait, timeout):
    ...
    try:
        if wait:
            click.echo(
                f"⏳ Waiting for task {task_id} completion (timeout: {timeout}s)..."
            )
            ...
            while time.time() - start_time < timeout:
                ...
                click.echo(
                    click.style("⏱️ Timeout waiting for task completion", fg="yellow")
                )
        ...
        if response.status_code == 404:
            ...
            click.echo(
                click.style(
                    f"❌ Failed to fetch task: HTTP {response.status_code}", fg="red"
                )
            )
            sys.exit(1)
        ...
        if verbose:
            ...
            click.echo(
                f"   Description: {task_data.get('description', 'No description')[:50]}..."
            )
...
def list(ctx, crew_id, status, limit):
    ...
    try:
        ...
        for task in tasks:
            ...
            click.echo(
                f"{status_icon} {task_id} | {description}... | {status} | {priority}"
            )
