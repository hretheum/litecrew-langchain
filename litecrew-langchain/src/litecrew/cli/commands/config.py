...
@click.option('--format', 'output_format', type=click.Choice(['json', 'yaml']), default='yaml', help='Configuration format')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--template', type=click.Choice(['basic', 'advanced', 'production']), default='basic', help='Configuration template')
def init(output_format, output, template):
    ...
    if output_format == 'json':
        ...
        formatted_config = yaml.dump(config_data, default_flow_style=False, sort_keys=False)
    ...
    try:
        ...
        click.echo(click.style(f"✅ Configuration initialized: {output_path}", fg='green'))
        ...
        click.echo(click.style(f"❌ Failed to create configuration: {str(e)}", fg='red'))
...
def validate(config_file):
    ...
    try:
        ...
        if 'logging' in config_data:
            ...
            if 'level' in logging_config:
                ...
                if level not in valid_levels:
                    issues.append(f"Invalid log level: {level}. Valid levels: {valid_levels}")
        ...
        if 'defaults' in config_data:
            ...
            if 'crew_process' in defaults:
                ...
                if process not in valid_processes:
                    issues.append(f"Invalid crew process: {process}. Valid: {valid_processes}")
        ...
        # Display results
        if issues:
            click.echo(click.style(f"❌ Validation failed ({len(issues)} error(s)):", fg='red'))
            for issue in issues:
                click.echo(f"   • {issue}")
        else:
        ...
        if 'logging' in config_data:
            click.echo(f"   Log Level: {config_data['logging'].get('level', 'Default')}")
        
        if 'defaults' in config_data:
            click.echo(f"   Default Process: {config_data['defaults'].get('crew_process', 'sequential')}")
...
def convert(source_file, target_format, output):
    ...
    try:
        ...
        click.echo(click.style(f"✅ Converted {source_format} → {target_format}: {output_path}", fg='green'))
