    ...
    mock_logs = [
        {
            ...
            "level": "info",
            "message": "LiteCrew API server started",
        },
        {
            "timestamp": datetime.now() - timedelta(minutes=4),
            "level": "info",
            "message": "Loaded configuration from config.yaml",
        },
        {
            "timestamp": datetime.now() - timedelta(minutes=3),
            "level": "warn",
            "message": "High memory usage detected: 45MB",
        },
        {
            "timestamp": datetime.now() - timedelta(minutes=2),
            "level": "info",
            "message": "Crew 'research-team' created successfully",
        },
        {
            "timestamp": datetime.now() - timedelta(minutes=1),
            "level": "info",
            "message": "Task execution completed in 2.3s",
        },
        {
        ...
        },
        {
            ...
            "level": "debug",
            "message": "Processing request ID: req_123456",
        },
    ]
            ...
            while True:
                ...
                if not level or new_entry["level"] == level:
                    ...
                    level_display = click.style(
                        f"[{new_entry['level'].upper()}]", fg="blue"
                    )
                    click.echo(
                        f"{timestamp_str} {level_display} {new_entry['message']}"
                    )
...
def connectivity(ctx):
    ...
    if passed == total:
        ...
        failed = total - passed
        click.echo(
            click.style(f"⚠️ {failed}/{total} components have issues", fg="yellow")
        )
...
def trace(ctx, crew_id):
    ...
    try:
        # Get crew details
        with httpx.Client() as client:
            crew_response = client.get(
                f"{api_url}/api/v1/crews/{crew_id}", timeout=10.0
            )
        ...
        if crew_response.status_code == 404:
            ...
            click.echo(
                click.style(
                    f"❌ Failed to fetch crew: HTTP {crew_response.status_code}",
                    fg="red",
                )
            )
            sys.exit(1)
        ...
        try:
            with httpx.Client() as client:
                executions_response = client.get(
                    f"{api_url}/api/v1/crews/{crew_id}/executions", timeout=10.0
                )
            ...
            if executions_response.status_code == 200:
                ...
                if executions:
                    ...
                    for exec_data in executions[:5]:  # Show last 5
                        ...
                        click.echo(
                            f"   {status_icon} {exec_id} | {status} | {duration:.1f}s"
                        )
        except Exception:
