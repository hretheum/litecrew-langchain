...
class MemorySearch:
    ...
    def find_related_turns(
        ...
        for i, turn in enumerate(turns):
            ...
            if score > 0:
                ...
                result["relevance_score"] = score
                result["turn_index"] = i
                result["relation_type"] = self._determine_relation_type(target_turn, turn)
    ...
    def _score_turn(self, turn: Dict[str, Any], query_tokens: List[str]) -> float:
        ...
        # Combine scores
        final_score = (exact_score * 0.5 + partial_score * 0.3 + tf_score * 0.2) * role_weight
