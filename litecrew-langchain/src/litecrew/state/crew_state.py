...
class CrewState:
    ...
    @classmethod
    def from_crew(cls, crew_id: str, agents: List[Any], tasks: List[Any], 
                  process: str = "sequential") -> 'CrewState':
        ...
        for agent in agents:
            agent_data.append({
                ...
                "llm": agent.llm.__class__.__name__ if hasattr(agent, 'llm') else "Unknown"
            })
        ...
        for task in tasks:
            task_data.append({
                ...
                "context_indices": [tasks.index(t) for t in (getattr(task, 'context', []) or [])]
            })
    ...
    def update_task_status(self, task_index: int, status: str) -> None:
        ...
        if 0 <= task_index < len(self.task_states):
            ...
            if status == "in_progress" and self.task_states[task_index]["start_time"] is None:
    ...
    def get_completed_tasks(self) -> List[int]:
        ...
        return [
            i for i, state in enumerate(self.task_states)
