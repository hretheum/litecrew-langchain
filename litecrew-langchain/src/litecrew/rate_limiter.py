...
class GlobalRateLimiter:
    ...
    def __init__(
        self, max_rpm: int = 1000, per_agent_limits: Optional[Dict[str, int]] = None
    ):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if handle_rate_limits and "rate limit" in error_msg:
                        ...
                        delay = min(
                            base_delay * (exponential_base**attempt), max_delay / 2
                        )
                        await asyncio.sleep(delay)
    ...
    def track_cost(self, agent_id: str, cost: float):
        ...
        if self.hard_limit and total_spent >= self.daily_limit:
            if "limit_exceeded" not in self._alerts_sent:
                ...
                if self.alert_callback:
                    self.alert_callback(
                        "Daily budget limit exceeded", total_spent, self.daily_limit
                    )
