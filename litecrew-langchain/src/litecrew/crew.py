...
class LiteCrew:
    ...
    def _execute_sequential(
            ...
            try:
                ...
                if self.lifecycle_callbacks:
                    self.lifecycle_callbacks.trigger(
                        "crew_task_complete", {"task": task, "output": output}
                    )
        ...
        # Return final output
        return CrewOutput(raw=outputs[-1].raw if outputs else "", tasks_output=outputs)
...
Respond with just the agent's role."""
            ...
            assigned_agent = next(
                (
                    agent
                    for agent in self.agents
                    if agent.role.lower() in assigned_role.lower()
                ),
            ...
            progress_update = (
                f"Task completed by {assigned_agent.role}: {output.raw[:100]}..."
            )
    ...
    async def _aexecute_hierarchical(
        self, inputs: Optional[Dict[str, Any]] = None
    ) -> CrewOutput:
    ...
    async def kickoff_async(
        ...
        return {
            ...
            "tasks_output": [
                {
                    "raw": output.raw,
                    "agent": getattr(output, "agent_role", "unknown"),
                    "task": getattr(output, "task_id", "unknown"),
                }
