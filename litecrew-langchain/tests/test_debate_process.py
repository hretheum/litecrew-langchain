...
class TestDebateProcess:
    ...
    async def test_basic_debate(self):
        ...
        # Setup agents
        proponent = LiteAgent(role="Advocate", goal="Support", backstory="Policy expert")
        opponent = LiteAgent(role="Critic", goal="Challenge", backstory="Devil's advocate")
    ...
    async def test_debate_with_moderator(self):
        ...
        # Setup agents
        moderator = LiteAgent(role="Moderator", goal="Facilitate", backstory="Neutral party")
    ...
    async def test_debate_rounds(self):
        ...
        argument_turns = [t for t in result.turns if t.metadata.get('phase') == 'argument']
    ...
    async def test_rebuttals(self):
        ...
        # Check for rebuttals
        rebuttal_turns = [t for t in result.turns if t.metadata.get('phase') == 'rebuttal']
        ...
        rebuttal_turns2 = [t for t in result2.turns if t.metadata.get('phase') == 'rebuttal']
