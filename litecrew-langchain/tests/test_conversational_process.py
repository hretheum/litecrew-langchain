...
class TestConversationalProcess:
    ...
    async def test_basic_conversation(self):
        ...
        task = LiteTask(
            ...
            expected_output="Feature specification"
        )
        
        # Mock agent responses
        analyst.execute = Mock(return_value="From an analytics perspective, we need tracking.")
        designer.execute = Mock(return_value="The UI should be intuitive and accessible.")
        developer.execute = Mock(return_value="Implementation completed with modular architecture.")
    ...
    async def test_task_extraction(self):
        ...
        responses = {
            "Worker": ["Let me work on implementing this feature", "I've completed the implementation of the feature with all tests passing."],
            "Reviewer": ["I'll review once you're done", "Great work! The implementation looks solid."]
        }
        ...
        assert ("completed" in task_outputs_text or "implementation" in task_outputs_text or
                "let me work" in conversation_text or len(result.turns) >= 4)
    ...
    async def test_conversation_phases(self):
        ...
        # Verify order
        intro_turn = next(t for t in result.turns if t.metadata.get('phase') == 'introduction')
        summary_turn = next(t for t in result.turns if t.metadata.get('phase') == 'summary')
