...
from litecrew.task import LiteTask, Task, TaskOutput
...
class TestTaskOutput:
    ...
    def test_task_output_creation(self):
        ...
        output = TaskOutput(
            raw="Test output", task_id="task-123", agent_role="TestAgent"
        )
...
class TestLiteTask:
    ...
    def test_task_creation_basic(self):
        ...
        task = LiteTask(description="Test task", expected_output="Expected result")
        ...
        assert not task.async_execution
    ...
    def test_task_execution_with_callback(self):
        ...
        task = LiteTask(
            description="Test task", agent=mock_agent, callback=mock_callback
        )
    ...
    def test_task_execution_with_task_context(self):
        ...
        prev_task = LiteTask(description="Previous task", agent=prev_agent)
        prev_task.output = TaskOutput(raw="Previous result", agent_role="PrevAgent")
        ...
        task = LiteTask(
            description="Current task", agent=mock_agent, context=[prev_task]
        )
    ...
    def test_build_context_with_tasks_no_output(self):
        ...
        task = LiteTask(description="Current task", context=[prev_task])
    ...
    def test_task_str_representation(self):
        ...
        assert str_repr.startswith(
            "Task: This is a very long task description that should"
        )
