...
class TestLLMManager:
    ...
    def test_create_anthropic_llm(self, mock_create):
        ...
        manager = LLMManager()
        config = LLMConfig(
            provider=LLMProvider.ANTHROPIC, model="claude-3-sonnet", api_key="test-key"
        )
    ...
    def test_create_groq_llm(self, mock_create):
        ...
        manager = LLMManager()
        config = LLMConfig(
            provider=LLMProvider.GROQ, model="mixtral-8x7b", api_key="test-key"
        )
    ...
    def test_error_handling_in_creation(self, mock_create):
        ...
        manager = LLMManager()
        config = LLMConfig(
            provider=LLMProvider.OPENAI, model="gpt-4", api_key="invalid-key"
        )
