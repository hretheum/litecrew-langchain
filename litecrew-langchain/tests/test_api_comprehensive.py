...
from litecrew.api.storage import get_storage, APIStorage
...
class TestCrewEndpoints:
    ...
    def test_execute_crew(self, client, auth_headers, sample_crew_data):
        ...
        with patch('litecrew.crew.LiteCrew.kickoff') as mock_kickoff:
            ...
            mock_output = CrewOutput(
                ...
                tasks_output=[
                    TaskOutput(raw="Task 1 done", task_id="1", agent_role="Lead Researcher"),
                    TaskOutput(raw="Task 2 done", task_id="2", agent_role="Data Analyst")
                ]
            )
    ...
    def test_get_crew_executions(self, client, auth_headers, sample_crew_data):
        ...
        # Get executions
        response = client.get(
            f"/api/v1/crews/{crew_id}/executions",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
...
class TestProcessEndpoints:
    ...
    def test_get_process_type_details(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/process-types/sequential",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
        ...
        response = client.get(
            "/api/v1/process-types/nonexistent",
            headers=auth_headers
        )
        assert response.status_code == 404
...
class TestTemplateEndpoints:
    ...
    def test_get_template_by_id(self, client, auth_headers):
        ...
        if templates:
            ...
            response = client.get(
                f"/api/v1/templates/quickstart/{template_id}",
                headers=auth_headers
            )
            assert response.status_code == 200
            data = response.json()
    ...
    def test_create_crew_from_template(self, client, auth_headers):
        ...
        with patch('litecrew.api.template_storage.TemplateStorage.get_template') as mock_get:
...
class TestUtilityEndpoints:
    ...
    def test_generate_crew_visualization(self, client, auth_headers, sample_crew_data):
        ...
        # Get visualization
        response = client.get(
            f"/api/v1/crews/{crew_id}/visualization",
            headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_export_crew_config(self, client, auth_headers, sample_crew_data):
        ...
        # Export as YAML
        response = client.get(
            f"/api/v1/crews/{crew_id}/export?format=yaml",
            headers=auth_headers
        )
        assert response.status_code == 200
        assert response.headers["content-type"].startswith("text/yaml") or "yaml" in response.text
        
        # Export as JSON
        response = client.get(
            f"/api/v1/crews/{crew_id}/export?format=json",
            headers=auth_headers
        )
        assert response.status_code == 200
...
class TestErrorHandling:
    ...
    def test_validation_error(self, client, auth_headers):
        ...
        invalid_data = {
            "role": "Agent"  # Missing goal and backstory
        }
        response = client.post(
            "/api/v1/agents",
            json=invalid_data,
    ...
    def test_internal_server_error(self, client, auth_headers):
        ...
        with patch('litecrew.api.routers.agents.Agent') as mock_agent:
            ...
            response = client.post("/api/v1/agents", json={"role": "Test", "goal": "Test", "backstory": "Test"}, headers=auth_headers)
...
class TestAPIModels:
    ...
    def test_crew_create_model(self):
        ...
        data = {
            "name": "Team",
            "agents": [
                {"role": "A1", "goal": "G1", "backstory": "B1"}
            ],
            "tasks": [
                {"description": "T1", "expected_output": "O1"}
            ]
        }
