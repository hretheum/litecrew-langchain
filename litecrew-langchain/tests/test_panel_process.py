...
class TestPanelProcess:
    ...
    async def test_panel_with_voting(self):
        ...
        task = LiteTask(
            description="Choose best approach", expected_output="Voted decision"
        )
        ...
        process = ProcessFactory.create(
            "panel", {"voting_enabled": True, "consensus_required": False}
        )
    ...
    async def test_panel_consensus(self):
        ...
        assert result.success is True
        consensus_turns = [
            t for t in result.turns if t.metadata.get("phase") == "consensus"
        ]
    ...
    async def test_panel_follow_ups(self):
        ...
        follow_up_turns = [
            t for t in result.turns if t.metadata.get("phase") == "follow_up"
        ]
        assert len(follow_up_turns) >= 4  # At least 2 rounds * 2 panelists

        # Check round numbers
        rounds = {
            t.metadata.get("round") for t in follow_up_turns if "round" in t.metadata
        }
        assert len(rounds) >= 2
    ...
    async def test_moderator_questions(self):
        ...
        topic_intros = [
            t for t in result.turns if t.metadata.get("phase") == "topic_introduction"
        ]
        ...
        topic_intros2 = [
            t for t in result2.turns if t.metadata.get("phase") == "topic_introduction"
        ]
