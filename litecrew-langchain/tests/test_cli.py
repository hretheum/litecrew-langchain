...
class TestCLIBasics:
    ...
    def test_cli_with_config(self, runner, temp_config_file):
        ...
        result = runner.invoke(cli, ['--config', temp_config_file, '--verbose', '--help'])
        assert result.exit_code == 0
        assert "LiteCrew" in result.output
...
class TestStatusCommand:
    ...
    def test_status_success(self, mock_client, runner):
        ...
        mock_client_instance.get.side_effect = [mock_health_response, mock_crews_response]
...
class TestTaskCommands:
    ...
    def test_task_run_success(self, mock_client, runner):
        ...
        result = runner.invoke(cli, [
            'task', 'run', 'Test task description',
            '--crew-id', 'test-crew',
            '--priority', 'high'
        ])
        assert result.exit_code == 0
...
def test_cli_integration_workflow(runner, temp_crew_file):
    ...
    with patch('httpx.Client') as mock_client:
        ...
        mock_responses = {
            'health': Mock(status_code=200, json=lambda: {"status": "healthy", "memory_mb": 25}),
            'crews': Mock(status_code=200, json=lambda: {"crews": []}),
            'create': Mock(status_code=201, json=lambda: {"crew_id": "test-123", "name": "Test Crew", "agents": [], "tasks": []}),
            'execute': Mock(status_code=200, json=lambda: {"execution_id": "exec-123", "status": "completed", "result": {}})
        }
