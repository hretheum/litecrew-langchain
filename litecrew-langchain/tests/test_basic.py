...
def test_task_creation():
    ...
    task = LiteTask(
        description="Write a blog post", agent=agent, expected_output="A blog post"
    )
...
def test_crew_creation():
    ...
    research_task = LiteTask(description="Research AI frameworks", agent=researcher)

    write_task = LiteTask(
        description="Write blog post", agent=writer, context=[research_task]
    )
...
def test_task_context_building():
    ...
    task2 = LiteTask(description="Second task", context=[task1])
...
def test_auto_task_assignment():
    ...
    LiteCrew(agents=[agent1, agent2], tasks=[task1, task2, task3])
...
def test_hierarchical_needs_manager():
    ...
    crew = LiteCrew(agents=[agent], tasks=[task], process="hierarchical")
