...
class TestStateManager:
    ...
    def test_concurrent_state_access(self, manager):
        ...
        def update_state(task_id):
            try:
                time.sleep(0.01 * task_id)  # Small delay to increase chance of conflicts
                # Load state
...
def test_state_metrics():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(10):
            ...
            state = CrewState(
                crew_id=f"crew_{i}",
                agents=[{"role": f"agent_{j}", "data": "x" * (j * 100)} for j in range(i + 1)],
                tasks=[{"description": f"task_{j}", "data": "y" * (j * 100)} for j in range(i + 1)],
                process="sequential"
            )
            ...
            start = time.perf_counter()
            restored = manager.load_state(f"crew_{i}")
