...
def test_cache_integration():
    ...
    cache = MultiLevelCache(l1_size=100, l2_size=1000, l3_enabled=True, policy=policy)
    ...
    # Cache crew state
    cache.set(
        f"crew:{crew_id}:state",
        {"status": "running", "tasks_completed": 5, "agents": ["researcher", "writer"]},
    )

    # Cache task results
    for i in range(10):
        cache.set(
            f"result:{crew_id}:task_{i}",
            {"output": f"Task {i} completed", "duration": 2.5 + i * 0.1},
        )
    ...
    def compute_analytics():
        ...
        cache.set(
            f"crew:{crew_id}:analytics",
            {"total_duration": total_duration, "average_duration": total_duration / 10},
        )
