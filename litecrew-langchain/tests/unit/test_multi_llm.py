...
class TestMultiLLMSupport:
    ...
    def test_agent_with_custom_llm_provider(self):
        ...
        assert agent.llm.__class__.__name__ in [
            "ChatAnthropic",
            "MockChatModel",
            "Mock",
            "MagicMock",
    ...
    def test_llm_creation_anthropic(self):
        ...
        manager = LLMManager()
        config = LLMConfig(
            provider=LLMProvider.ANTHROPIC, model="claude-3", temperature=0.5
        )
    ...
    def test_llm_creation_groq(self):
        ...
        manager = LLMManager()
        config = LLMConfig(
            provider=LLMProvider.GROQ, model="mixtral-8x7b", temperature=0.3
        )
    ...
    def test_llm_creation_ollama(self):
        ...
        manager = LLMManager()
        config = LLMConfig(provider=LLMProvider.OLLAMA, model="llama2", temperature=0.8)
    ...
    def test_llm_fallback_chain(self):
        ...
        assert agent._fallback_providers == [
            LLMProvider.OPENAI,
    ...
    def test_provider_specific_optimizations(self):
        ...
        openai_config = LLMConfig(
            provider=LLMProvider.OPENAI, model="gpt-4", use_functions=True
        )
