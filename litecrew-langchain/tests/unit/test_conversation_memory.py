...
from litecrew.memory import ConversationMemory, MemorySearch, MemorySummarizer
...
class TestConversationMemory:
    ...
    def test_memory_summarization(self):
        ...
        memory.add_turn(
            "assistant",
            "Python features include: dynamic typing, garbage collection...",
        )
    ...
    def test_agent_with_memory(self):
        ...
        # First interaction
        agent.execute("My name is Alice")

        # Second interaction should remember
        agent.execute("What is my name?")
    ...
    def test_crew_with_shared_memory(self):
        ...
        agent1 = LiteAgent(
            role="Agent 1", goal="First agent", backstory="Agent one", memory=True
        )

        agent2 = LiteAgent(
            role="Agent 2", goal="Second agent", backstory="Agent two", memory=True
        )
        ...
        crew = LiteCrew(
            agents=[agent1, agent2], tasks=[task], memory=True  # Enable shared memory
        )
        ...
        assert agent1._use_crew_memory
        assert agent2._use_crew_memory

    def test_memory_with_summarization_trigger(self):
        """Test automatic summarization when memory gets large."""
        memory = ConversationMemory(max_size=10, summarize_after=5)
