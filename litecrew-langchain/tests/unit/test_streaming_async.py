...
class TestStreamingAsync:
    ...
    async def test_batch_processing(self):
        ...
        assert duration < len(tasks) * 5.0  # Assuming 5s per task in CI
    ...
    async def test_first_token_latency(self):
        ...
        assert first_token_time < 10000  # 10 seconds for CI environments
    ...
    async def test_streaming_overhead(self):
        ...
        streaming_agent = LiteAgent(
            role="Streaming", goal="Stream", backstory="Streams", streaming=True
        )

        regular_agent = LiteAgent(
            role="Regular", goal="Regular", backstory="Regular", streaming=False
        )
    ...
    async def test_concurrent_agents(self):
        ...
        start = time.time()
        tasks = [
            agent.aexecute(f"Task for agent {i}") for i, agent in enumerate(agents)
        ]
        ...
        assert duration < 30.0  # 30s for concurrent execution in CI

    @pytest.mark.asyncio
    async def test_crew_async_execution(self):
        """Test crew async execution."""
        agent1 = LiteAgent(
            role="Async Agent 1", goal="First async agent", backstory="First agent"
        )

        agent2 = LiteAgent(
            role="Async Agent 2", goal="Second async agent", backstory="Second agent"
        )

        task1 = LiteTask(description="First async task", agent=agent1)

        task2 = LiteTask(description="Second async task", agent=agent2)

        crew = LiteCrew(
            agents=[agent1, agent2], tasks=[task1, task2], async_execution=True
        )
    ...
    async def test_batch_with_different_providers(self):
        ...
        agents = [
            LiteAgent(role="Agent 1", goal="First task", backstory="First agent"),
            LiteAgent(role="Agent 2", goal="Second task", backstory="Second agent"),
            LiteAgent(role="Agent 3", goal="Third task", backstory="Third agent"),
        ]
        ...
        assert duration < 60.0  # 60s for 3 concurrent tasks in CI
