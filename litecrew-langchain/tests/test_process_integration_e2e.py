...
class TestProcessIntegrationE2E:
    ...
    def test_list_process_types(self, client):
        ...
        assert len(process_types) >= 5  # sequential, hierarchical, conversational, debate, panel
    ...
    async def test_websocket_process_events(self, client, sample_crew_data):
        ...
        async def mock_websocket_handler():
            async with websockets.connect(f"ws://localhost:8000/ws/crew/{crew_id}") as websocket:
                # Listen for events
                try:
                    while True:
