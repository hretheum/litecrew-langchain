...
class TestProcessIntegrationE2E:
    ...
    def test_get_process_type_details(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/process-types/conversational", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_create_crew_with_process_config(
        self, client, sample_crew_data, auth_headers
    ):
    ...
    @pytest.mark.asyncio
    async def test_websocket_process_events(
        self, client, sample_crew_data, auth_headers
    ):
        ...
        asyncio.create_task(
            client.post(
                f"/api/v1/crews/{crew_id}/execute", json={}, headers=auth_headers
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_process_performance_metrics(
        self, client, sample_crew_data, auth_headers
    ):
